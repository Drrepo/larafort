...
`Larafort` provides a simple and rich way to perform and handle operations for 
...
After installing the `Larafort` library, register the `LaravelPayfort\Providers\PayfortServiceProvider` 
...
`Larafort` is open-sourced software licensed under the [MIT license](http://opensource.org/licenses/MIT)
